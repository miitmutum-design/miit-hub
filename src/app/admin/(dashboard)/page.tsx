...
'use client';
import { Building2, Star, Shapes, Clock, CheckCircle, XCircle, BarChart3, Activity } from 'lucide-react';
  ...
  return (
    <main className="flex flex-1 flex-col gap-4 p-4 sm:px-6 sm:py-0 md:gap-8">
        ...
        <div className="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
            ...
            >
                <Link href="/admin/empresas?filter=pending" className="mt-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
            ...
            <Card className="lg:col-span-4">
                ...
                <CardContent className="grid gap-8">
                    <div>
                        <h3 className="text-sm font-medium text-muted-foreground mb-2">Empresas para Aprovar</h3>
                    ...
                    <div>
                        <h3 className="text-sm font-medium text-muted-foreground mb-2">Sugestões de Categorias</h3>
                         <div className="space-y-3">
                             {categorySuggestions.map(cat => (
                                <div key={cat.name} className="flex items-center justify-between p-2 bg-muted/50 rounded-md">
                                    <p className="text-sm font-medium">{cat.name}</p>
                                    <div className="flex items-center gap-2">
                                        <Button variant="ghost" size="icon" className="h-7 w-7 text-green-500"><CheckCircle className="w-4 h-4"/></Button>
                                        <Button variant="ghost" size="icon" className="h-7 w-7 text-red-500"><XCircle className="w-4 h-4"/></Button>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <Card className="lg:col-span-3">
                ...
                <CardContent className="grid gap-8">
                    <div>
                     ...
                     <div>
                        <h3 className="text-sm font-medium text-muted-foreground mb-2">Status das Ofertas</h3>
                        <div className="flex items-center justify-around text-center p-2 bg-muted/50 rounded-md">
                           <div>
